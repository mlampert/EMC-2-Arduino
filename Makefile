...
AVR_TOOLS_PATH ?= /opt/arduino/hardware/tools/avr/bin/
AVRDUDE_PROGRAM ?= /opt/arduino/hardware/tools/avr/bin/avrdude
AVRDUDE_CONFIG_FILE ?= /opt/arduino/hardware/tools/avr/etc/avrdude.conf

# This defines the board to compile for (see boards.h for your board's ID)
HARDWARE_MOTHERBOARD ?= 777
...
# Infinity
ifeq  ($(HARDWARE_MOTHERBOARD),774)
HARDWARE_VARIANT ?= arduino
MCU              ?= atmega2560
else ifeq  ($(HARDWARE_MOTHERBOARD),775)
HARDWARE_VARIANT ?= arduino
MCU              ?= atmega2560
else ifeq  ($(HARDWARE_MOTHERBOARD),776)
HARDWARE_VARIANT ?= arduino
MCU              ?= atmega2560
else ifeq  ($(HARDWARE_MOTHERBOARD),777)
HARDWARE_VARIANT ?= arduino
MCU              ?= atmega2560
endif
...
CXXSRC = WMath.cpp WString.cpp Print.cpp  \
	 SPI.cpp  Tone.cpp HardwareSerial.cpp HardwareSerial0.cpp HardwareSerial1.cpp HardwareSerial2.cpp HardwareSerial3.cpp EmcArduino.cpp 
...
AVRDUDE_CONF = $(AVRDUDE_CONFIG_FILE)
else
...
$(BUILD_DIR)/%.o: $(BUILD_DIR)/%.cpp  $(MAKEFILE)
	$(Pecho) "  CXX   $<"
